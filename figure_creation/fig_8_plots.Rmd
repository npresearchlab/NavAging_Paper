...
df_merge <- merge(nav_data,non_nav_data,by="Participant") 
ya_data <- df_merge %>%
  filter(Age == "YA")
...
p_ct <- ggplot(data, aes(x=Block_num, y=Total_Time, color=Cohort))+
  scale_color_manual(values =c('#5E7AFA', '#FF9FAF'))+
  geom_line(data, mapping = aes(x= Block_num, y = Total_Time, group = Participant, color = Cohort), alpha = .2)+
  geom_point(data, mapping = aes(x= Block_num, y = Total_Time, group = Participant), color = 'white', size = 5)+
  geom_point(data, mapping = aes(x= Block_num, y = Total_Time, group = Cohort, color = Cohort), size = 3, alpha = .2)+

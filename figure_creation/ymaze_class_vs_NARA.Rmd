...
```{r}
NARA_ymaze_class_age <- ggplot(non_nav_data, aes(x=ymaze_class_age_presence, y=NARA, color=Participant))+
 geom_point(size=4)+
...
```{r}
NARA_ymaze_oa <- ggplot(non_nav_data, aes(x=ymaze_class_oa_presence, y=NARA, color=Participant))+
 geom_point(size=4)+
  stat_summary(fun.args = list(mult=1), 
                geom="crossbar", 
                width=0.2, 
  ...
  #scale_color_manual(values =c( '#FF9FAF', '#5E7AFA','#9dff00'))+
  ggtitle("X")

NARA_ymaze_oa 
```
```{r}
NARA_ymaze_class <- ggplot(non_nav_data, aes(x=ymaze_class, y=NARA, color=Participant))+
 geom_point(size=4)+
  stat_summary(fun.args = list(mult=1), 
                geom="crossbar", 
                width=0.2, 
                color="purple")+
  labs(color= "Participant", y="NARA", x="Age-Reference Frame")+
  scale_x_discrete("X",limits = c("ego","allo"))+
  #scale_color_manual(values =c( '#FF9FAF', '#5E7AFA','#9dff00'))+
  ggtitle("X")

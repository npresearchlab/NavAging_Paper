...
#ggsave(file="~/Desktop/NPRL (RS)/NAVIGATION/NavAging_Paper/figure_creation/plots/fig6_corr_nara.svg", height = 5, width = 8, plot=corr_nara)
```
...
non_nav_ya_data <- non_nav_data_final %>% filter(group == 'YA')
...
non_nav_oa_data <- non_nav_data_final %>% filter(group == 'OA')
...
```{r}
#non_nav_oa_high_data <- subset(non_nav_oa_data, nara > 50.00)
#non_nav_oa_low_data <- subset(non_nav_oa_data, nara < 50.00)

corr_nara_three <-ggplot(non_nav_data_final, aes(x=age, y=nara, color = nara_age))+
  geom_point(size=2)+
  #labs(color = c('#FF9FAF','#5E7AFA'))+
  ...
  theme(axis.title.y = element_text(size=20))+
  theme(axis.title.x = element_text(size=20))+
  geom_smooth(aes(age, nara, group=nara_age), method="lm", se=TRUE, color = "#242526", fill = 'lightgray')
 
corr_nara_three

ggsave(file="~/Desktop/NPRL (RS)/NAVIGATION/NavAging_Paper/figure_creation/plots/fig6_corr_nara_three.svg", height = 5, width = 8, plot=corr_nara_three)
```

```{r}
non_nav_oa_high_data <- non_nav_data_final %>% filter(nara_age == 'OA_high')
...
non_nav_oa_low_data <- non_nav_data_final %>% filter(nara_age == 'OA_low')

...
change_data <- nav_data %>%
  ...
  select(Participant, Block_Num, Total_Time, Orientation_Time, 
         Navigation_Time, Distance, Speed, Mean_Dwell, Teleportations) %>%
...
plot_specs <- tribble(
  ...
  "Distance",          "Distance Traveled (VR Units)", NULL,        seq(0, 1200, 200),
  "Speed",             "Speed (VR Unit/s)",           NULL,        seq(0, 30, 5),
...
names(plots) <- paste0("p_", c("ct", "orict", "navct", "dt", "speed", "dwell", "teleport"))
...
plot_names <- c("fig3_ct", "fig3_orict", "fig3_navct", "fig3_dt", 
                "fig3_speed", "fig3_dwell", "fig3_teleport")
...
# 4 columns, 2 rows layout (with one empty space)
...
save_plot(composite_plot, "fig3_composite", width = 20, height = 16)
```
...
```{r optional-change-analysis, eval=FALSE, include=FALSE}
# Optional: Analyze change scores from Block 1 to Block 3
# Uncomment and run this chunk if you want to examine learning effects

change_summary <- change_data %>%
  # Add age information back
  left_join(
    nav_data %>% select(Participant, Age) %>% distinct(),
    by = "Participant"
  ) %>%
  
  # Calculate summary statistics for change scores
  pivot_longer(
  ...
  summarise(
    ...
    se_change = round(std.error(Change_Score), 3),
    significant_improvement = mean_change < 0,  # Negative = improvement for most measures
    .groups = 'drop'
  )

print("Change scores from Block 1 to Block 3:")

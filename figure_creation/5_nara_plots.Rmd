---
title: "NARA Plots Code - Optimized"
...
```{r}
# NEW CODE CHUNK HERE SAMPLE TEXT BLA BLA Bkjflskdjflksdj
...
nara_data <- non_nav_data %>%
  ...
  mutate(
    ...
    nara_labels = case_when(
      Group == "YA" ~ "YA",
      Group == "OA" & NARA >= 3.5 ~ "OA High", 
      Group == "OA" & NARA < 3.5 ~ "OA Low",
      TRUE ~ NA_character_
    ),
...
# Function to save plots with consistent settings as SVG
...
# Function to save plots with consistent settings as PNG
save_plot <- function(plot, filename, width = 5, height = 5) {
  ggsave(
...
create_nara_raincloud <- function(data, colors = c('#FF9FAF', '#5E7AFA')) {
  ggplot(data, aes(x = Group, y = NARA, fill = Group, color = Group)) +
    ...
    geom_beeswarm(
      aes(x = as.numeric(Group) - 0.3, y = NARA),
      side = 1,
      alpha = 0.5,
      cex = 1.5
    )
...
plot_nara_three <- create_correlation_plot(
  ...
  colors = c('#FF9FAF', '#325AF5', '#8A9AFF')  # YA, oa_high, oa_low
)
...
# Save all plots
save_plot(plot_nara_rain, "5_nara_rain")
save_plot(plot_nara_corr, "5_nara_corr")
save_plot(plot_nara_three, "5_nara_cohorts")
```
...
print("NARA summary statistics by three-group cohort classification:")

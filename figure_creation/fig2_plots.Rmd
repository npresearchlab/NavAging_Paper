...
p_dt <- create_raincloud_plot(
  mean_data, 
  "Distance", 
  "Mean Distance Traveled (VR Units)"
)
...
save_plot(p_ct, "fig2_ct")
save_plot(p_orict, "fig2_orict") 
save_plot(p_navct, "fig2_navct")
save_plot(p_dt, "fig2_dt")
save_plot(p_speed, "fig2_speed")
save_plot(p_dwell, "fig2_dwell")
save_plot(p_teleport, "fig2_teleport")
```
...
# 4 columns, 3 rows layout (with one empty space)
...
save_plot(composite_plot, "fig2_composite", width = 20, height = 16)
```
...
```{r optional-statistical-tests, eval=FALSE, include=FALSE}
# Optional: Run statistical tests comparing age groups
# Uncomment and run this chunk if you want statistical comparisons

library(broom)

# Function to run t-tests for each measure
run_comparisons <- function(data) {
  measures <- c("Total_Time", "Orientation_Time", "Navigation_Time", 
                "Distance", "Speed", "Mean_Dwell", "Teleportations")
  
  results <- map_dfr(measures, function(measure) {
    formula_str <- paste(measure, "~ Age")
    t_test_result <- t.test(as.formula(formula_str), data = data)
    
    tibble(
      Measure = measure,
      t_statistic = round(t_test_result$statistic, 3),
      p_value = round(t_test_result$p.value, 4),
      YA_mean = round(t_test_result$estimate[1], 2),
      OA_mean = round(t_test_result$estimate[2], 2),
...
# Run comparisons
statistical_results <- run_comparisons(mean_data)
print("Statistical comparisons between age groups:")

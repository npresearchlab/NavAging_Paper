...
ya_p_ct <-ggplot(allblocks_targets, aes(x = Target_Name, y = Total_Time_m, fill = Age)) +
  geom_bar_pattern(position = position_dodge(), stat = "identity", 
                   aes(pattern = Block_num), 
                   pattern_density = 0.05, 
                   pattern_spacing = 0.05,
                   pattern_fill = "white",
                   pattern_angle = 45) +
  scale_fill_manual(values =c('#5E7AFA', '#FF9FAF')) +
  scale_pattern_manual(values = c("1" = "stripe", "2" = "circle", "3" = "crosshatch")) +
  theme_minimal() +
  labs(title = "Bar Plot with Patterns by Age and Block",
       x = "Target Name",
       y = "Total Time (s)",
       fill = "Age",
...
```
  ...
  ggplot(allblocks_targets, aes(x=Target_Name, y=Total_Time_m, color= Block_num, fill = Age))+
  geom_bar(allblocks_targets, mapping = aes(x= Block_num, y = Total_Time_m,
                                             group = Target_Name, 
                                             color= Age), alpha = 0.7)+
  ...
  geom_point(ya_test_allblocks_targets, mapping = aes(x= Block_num, 
                                              ...
                                              fill = Target_Name),
             alpha = 0.7, size = 3)+
  geom_errorbar(ya_test_allblocks_targets, mapping = aes(x=Block_num, ymin=Total_Time_m-Total_Time_se, ymax=Total_Time_m+Total_Time_se, group = Target_Name, color = Target_Name),width=0.1, alpha=0.7)+

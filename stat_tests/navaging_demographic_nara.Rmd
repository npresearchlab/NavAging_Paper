...
## Does gender differ in YA vs OA high/low groups? 
...
# Video Game Experience vs NARA Category 
```{r}
...
# VR Experience vs NARA Category
```{r}

## Does VR experience differ in YA vs OA high/low groups? 

## chi-square test for VR experience quantified vs nara category

vr_nara <- data.frame(alldata$vr_exp_quantified,alldata$nara_group)
vr_nara <- table(alldata$vr_exp_quantified,alldata$nara_group)
print(vr_nara)

# running chi square with/without Yates correction

chisq.test(table(alldata$vr_exp_quantified, alldata$nara_group), correct = TRUE)
chisq.test(table(alldata$vr_exp_quantified, alldata$nara_group), correct = FALSE)

chisq.test(vr_nara, correct = TRUE)

# p = 0.07107, fail to reject H0
# chi square approx may be incorrect

```

# Corsi Span vs NARA Category
```{r}

## Does Corsi Span differ in YA vs OA high/low? 

# Run Shapiro-Wilks Test for Normality in YA/OA high/OA low Dataset for Corsi Span 

shapiro.test(ya_data$corsi_span)

# p = 0.001419, reject H0 & cannot assume normality

shapiro.test(oa_nara_high$corsi_span)

# p = 0.1227, fail to reject H0 & can assume normality

shapiro.test(oa_nara_low$corsi_span)

# p = 0.01541, reject H0 & cannot assume normality

# Run Bartlett's Test for Equal Variance

bartlett.test(alldata$corsi_span ~ alldata$nara_group)

# p = 0.538, fail to reject H0 & does exhibit equal variance

# Run Kruskal-Wallis Test as Nonparametric Alternative to One Way ANOVA

kruskal.test(corsi_span~nara_group, data = alldata)

# p = 0.0002137, significant difference in video game experience (in hours/week) for YA vs OA high vs OA low

```

# Corsi Mean Score vs NARA Category
```{r}

## Does Corsi Mean Score differ in YA vs OA high/low? 

# Run Shapiro-Wilks Test for Normality in YA/OA high/OA low Dataset for Corsi Span 

shapiro.test(ya_data$corsi_score_mean)

# p = 0.7147, fail to reject H0 & can assume normality

shapiro.test(oa_nara_high$corsi_score_mean)

# p = 0.6382, fail to reject H0 & can assume normality

shapiro.test(oa_nara_low$corsi_score_mean)

# p = 0.7782, fail to reject H0 & can assume normality

# Run Bartlett's Test for Equal Variance

bartlett.test(alldata$corsi_score_mean ~ alldata$nara_group)

# p = 0.3711, fail to reject H0 & does exhibit equal variance

# Run One Way ANOVA

### NEED TO RERUN 

kruskal.test(corsi_score_mean~nara_group, data = alldata)

# p = 1.02e-06, significant difference in corsi mean score for YA vs OA high vs OA low
...
## Does Corsi Total Score differ in YA vs OA high/low? 

# Run Shapiro-Wilks Test for Normality in YA/OA high/OA low Dataset for Corsi Span 

shapiro.test(ya_data$corsi_score_total)

# p = 0.7951, fail to reject H0 & can assume normality

shapiro.test(oa_nara_high$corsi_score_total)

# p = 0.6382, fail to reject H0 & can assume normality

shapiro.test(oa_nara_low$corsi_score_total)

# p = 0.7782, fail to reject H0 & can assume normality

# Run Bartlett's Test for Equal Variance

bartlett.test(alldata$corsi_score_total ~ alldata$nara_group)

# p = 0.3711, fail to reject H0 & does exhibit equal variance

# Run Kruskal-Wallis Test as Nonparametric Alternative to One Way ANOVA

kruskal.test(corsi_score_total~nara_group, data = alldata)

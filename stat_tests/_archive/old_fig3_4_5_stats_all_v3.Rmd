...
```{r}
# Jasmine's code
m_ct_emm <- emmeans(m_all_ct,~Age*Block_num)

m_ct_groups <- contrast(m_ct_emm, method = "pairwise")
m_ct_groups
```
...
table = data.frame(interaction_contrasts); b = data.frame(confint(interaction_contrasts))
table$lower.CL = b$lower.CL; table$upper.CL = b$upper.CL; table$D = abs(round((table$estimate)/(summary(m_all_ct)$sigma^2),digits = 2))
#gtsave(gt(table), paste0(imagesPath, "FOOOF_beta_PosturexFeedback_power_stats.png"))
```

```{r}
# Jasmine's code
m_ct_pairs <- emmeans(m_all_ct,~Age*Block_num)
m_ct_contrasts <- contrast(m_ct_pairs, method = "pairwise")
m_ct_contrasts
```

```{r}
m_ct_pairs <- emmeans(m_all_ct,~Age*Block_num*Target_Name)
m_ct_contrasts <- contrast(m_ct_pairs, method = "pairwise")
m_ct_contrasts
```
...
```{r}
# Running linear mi
m_ba_ct <- lmer(Total_Time ~ Block_num*Age + (1|Participant), data=all_data)
m_ba_ct_anova <- anova(m_ba_ct)
m_ba_ct_anova
# omnibus p-values - main effect of this variable
```
...
```{r}
# Jasmine's code
m_ct_pairs <- emmeans(m_ba_ct,~Age*Block_num)
m_ct_contrasts <- contrast(m_ct_pairs, method = "pairwise")
m_ct_contrasts
```
...
This R Markdown document extracts specific p-values from linear mixed model results for p-value extraction. The process involves:

1. Reading the p-value  specifications from `pval_template.xlsx`
2. Loading three CSV files containing contrast results of interest
...
```{r read-data}
# Read the pval template analysis file (updated filename)
...
```{r extract-pvalues}
# Apply the function to extract p-values for all rows in extracting p-value analysis
...
# Display the results
cat("Extracting P-Value Analysis Results:\n")
```
...
datatable(pvalue_extraction_results, 
          caption = "Extracted P-Value Results",

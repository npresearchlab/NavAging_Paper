...
# Load non-navigation data to get splitting criterion
...
# SPLIT OA GROUP BASED ON NARA SCORE (3.5 threshold)
# NARA >= 3.5 = OA_High (better navigation), NARA < 3.5 = OA_Low (worse navigation)
oa_subgroups <- non_nav %>%
  filter(Group == "OA") %>%
  mutate(Age_Group = ifelse(NARA >= 3.5, "OA_High", "OA_Low")) %>%
  select(Participant, Age_Group)

# Check the split
cat("OA group split based on NARA >= 3.5 threshold:\n")
print(table(oa_subgroups$Age_Group))

# Merge with navigation data
oa_data <- oa_data %>%
  left_join(oa_subgroups, by = "Participant")

# Combine all datasets
nav_data <- bind_rows(ya_data, oa_data)
...
# Age_Group factor levels: YA (reference) → OA_High → OA_Low
# This ordering tests:
# - OA_High - YA: How much navigation is impaired in high-functioning older adults?
# - OA_Low - YA: How much navigation is impaired in low-functioning older adults?  
# - OA_Low - OA_High: How much worse are low- vs high-functioning older adults?

nav_data$Age_Group <- factor(nav_data$Age_Group, 
                            levels = c("YA", "OA_High", "OA_Low"),
                            labels = c("YA", "OA_High", "OA_Low"))
...
cat("\n=== EXPECTED CONTRASTS ===\n")
cat("Age Group Contrasts (with current factor ordering):\n")
cat("- OA_High - YA: Expected negative (high-functioning OA worse than YA)\n")
cat("- OA_Low - YA: Expected more negative (low-functioning OA worse than YA)\n") 
cat("- OA_Low - OA_High: Expected negative (low worse than high within OA)\n\n")
...
# STEP 2: The rest of your code stays the same!
# The loop will automatically handle 3 groups and produce more contrasts

# STEP 3: Optional - Add specific contrasts of interest
# If you want to control which specific comparisons are made:
...
for (outcome in outcome_vars) {
  ...
  contrasts_interaction <- summary(pairs(emm_interaction, simple = "Age_Group", adjust = "fdr"), infer = TRUE)
  
  # Step 1: Get consecutive block changes within each age group  
  block_changes_within_age <- contrast(emm_interaction, "pairwise", simple = "Age_Group", combine = FALSE,
                                       adjust = "none")
  ...
  interaction_changes_df <- data.frame(
    ...
    Contrast_Type = "Block_change_difference_btwn_groups",
  ...
  # MERGE CONTRAST AND BLOCK_NUM COLUMNS: "OA_High - YA | Block1" format
  contrasts_interaction_df$contrast <- paste(
    contrasts_interaction_df$contrast, 
    contrasts_interaction_df$Block_Num, 
    sep = " | "
    )
  
  # Remove the separate Block_Num column
  contrasts_interaction_df <- contrasts_interaction_df %>% select(-Block_Num)
...
write.csv(all_anova_results, "all_anova_results.csv", row.names = FALSE)

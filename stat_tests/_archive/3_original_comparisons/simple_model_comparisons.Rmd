...
outcomes <- c("Total_Time", "Orientation_Time", "Navigation_Time", "Speed", 
              "Distance_Traveled", "Mean_Dwell", "Teleportations", 
...
for(outcome in outcomes) {
  ...
  formula_target_random <- paste(outcome, "~ Age * Block_Num + (1 | Participant) + (1 | Target_Name)")
  
  # Try to fit models with error handling
  tryCatch({
    
    # Model 1: Age × Block (your main learning model)
    model_block <- lmer(as.formula(formula_block), data = nav_data)
    
    # Model 2: Age × Target (fixed target effects)
    model_target_fixed <- lmer(as.formula(formula_target), data = nav_data)
    
    # Model 3: Age + Target (random target effects)
...
# cv_result <- cv_mixed_model(nav_data, "Navigation_Time ~ Age * Block + (1|Participant)")
# print(cv_result)
```

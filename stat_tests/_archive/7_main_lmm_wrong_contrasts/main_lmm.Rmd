...
for (outcome in outcome_list) {
 ...
 # Get pairwise contrasts with FDR correction (within outcome only)
  ...
  #Block 1 to Block 2 change difference between age groups
  block1vs2_age_diff <- contrast(emm_interaction,
                                 list("(OA_Block2-OA_Block1) vs (YA_Block2-YA_Block1)" = 
                                        c(-1, 1, 0, 1, -1, 0)),
                                 adjust = "fdr")
  #Block 1 to Block 3 change difference between age groups  
  block1vs3_age_diff <- contrast(emm_interaction,
                                 list("(OA_Block3-OA_Block1) vs (YA_Block3-YA_Block1)" = 
                                        c(-1, 0, 1, 1, 0, -1)),
                                 adjust = "fdr")
  # Block 2 to Block 3 change difference between age groups
  block2vs3_age_diff <- contrast(emm_interaction,
                                 list("(OA_Block3-OA_Block2) vs (YA_Block3-YA_Block2)" = 
                                        c(0, -1, 1, 0, 1, -1)),
                                 adjust = "fdr")
  ...
  # Convert interaction change contrasts to dataframes
  block1vs2_df <- data.frame(
    Outcome = outcome,
    Correction = "FDR_within_outcome",
    Contrast_Type = "Block1vs2_change_difference",
    as.data.frame(block1vs2_age_diff)
  )
  block1vs3_df <- data.frame(
    Outcome = outcome,
    Correction = "FDR_within_outcome", 
    Contrast_Type = "Block1vs3_change_difference",
    as.data.frame(block1vs3_age_diff)
  )
  block2vs3_df <- data.frame(
    Outcome = outcome,
    Correction = "FDR_within_outcome",
    Contrast_Type = "Block2vs3_change_difference", 
    as.data.frame(block2vs3_age_diff)
  )
  
  # Combine all interaction change contrasts
  interaction_changes_df <- rbind(block1vs2_df, block1vs3_df, block2vs3_df)
